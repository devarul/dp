...
class LitExtension_SocialLogin_Model_Google_Client
{
    const REDIRECT_URI_ROUTE = 'le_sociallogin/google/connect';
    const REDIRECT_URI_REQUEST = 'le_sociallogin/google/request';

    const XML_PATH_ENABLED = 'le_sociallogin/google/enabled';
    const XML_PATH_CLIENT_ID = 'le_sociallogin/google/api_key';
    const XML_PATH_CLIENT_SECRET = 'le_sociallogin/google/secret';
    ...
    {
        return Mage::getUrl('le_sociallogin/google/request', array("mainw_protocol" => $this->protocol));
    }
    ...
    protected function _httpRequest($url, $method = 'GET', $params = array())
    {
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401)) {
                ...
                throw new LitExtension_SocialLogin_GoogleOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('le_sociallogin')
...
class LitExtension_SocialLogin_GoogleOAuthException extends Exception
{}
