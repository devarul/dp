...
class LitExtension_SocialLogin_Model_Yahoo_Client
{
    const REDIRECT_URI_ROUTE = 'le_sociallogin/yahoo/connect';
    const REQUEST_TOKEN_URI_ROUTE = 'le_sociallogin/yahoo/request';

    const OAUTH_URI = 'https://api.login.yahoo.com/oauth/v2';
    const OAUTH2_SERVICE_URI = 'https://social.yahooapis.com/v1/user/';

    const XML_PATH_ENABLED = 'le_sociallogin/yahoo/enabled';
    const XML_PATH_CLIENT_ID = 'le_sociallogin/yahoo/api_key';
    const XML_PATH_CLIENT_SECRET = 'le_sociallogin/yahoo/secret';
    ...
    {
        return Mage::getUrl('le_sociallogin/yahoo/request', array("mainw_protocol" => $this->protocol));
    }
    ...
    protected function _httpRequest($url, $method = 'GET', $params = array())
    {
        ...
        if($response->isError()) {
            ...
            if(($status == 400 || $status == 401 || $status == 429)) {
                ...
                throw new LitExtension_SocialLogin_YahooOAuthException($message);
            } else {
                $message = sprintf(
                    Mage::helper('le_sociallogin')
...
class LitExtension_SocialLogin_YahooOAuthException extends Exception
{}
