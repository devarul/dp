...
class LitExtension_SocialLogin_Helper_Linkedin extends Mage_Core_Helper_Abstract
{
    ...
    public function disconnect(Mage_Customer_Model_Customer $customer) {
        ...
        $pictureFilename = Mage::getBaseDir(Mage_Core_Model_Store::URL_TYPE_MEDIA)
            ...
            .'linkedin'
            .DS
            .$customer->getLeSocialloginLid();

        if(file_exists($pictureFilename)) {
            @unlink($pictureFilename);
        }

            $customer->setLeSocialloginLid(null)
            ->setLeSocialloginLtoken(null)
            ->save();
    }
    ...
    public function connectByLinkedinId(
        ...
        $token)
    {
        $customer->setLeSocialloginLid($linkedinId)
            ->setLeSocialloginLtoken($token)
            ->save();
        ...
        $token)
    {
        ...
        $customer->setEmail($email)
            ...
            ->setLeSocialloginLid($linkedinId)
            ->setLeSocialloginLtoken($token)
    ...
    public function getCustomersByLinkedinId($linkedinId)
    {
        ...
        $collection = $customer->getCollection()
            ->addAttributeToFilter('le_sociallogin_lid', $linkedinId)
            ->setPageSize(1);
